...
3. `server.js` registers `sw.js`, which loads `npn.wasm`
...
- `./bin/build/wasmrelease.sh <version>`: Create `./build/dist/npn_<version>_wasm_html.zip`
- `./bin/wasmdev.sh`: Serve `./build/wasm` on `http://localhost:40009`
- `npn wasm`: Internal entrypoint used by the Service Worker (not for direct CLI use)
...
# upload build/dist/npn_1.2.3_wasm_html.zip to static hosting
```
