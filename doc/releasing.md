...
NPN uses `goreleaser` to create build artifacts.
...
Multiple Docker images will be created. The main image is `ghcr.io/kyleu/npn/x.x.x`, and a debug image is provided at `ghcr.io/kyleu/npn/x.x.x-debug` that includes `delve` for debugging
