...
func (t *Timing) CompleteHeaders() {
	t.ResponseHeaders = int((time.Now().UnixNano() - t.Began) / 1000)
}

func (t *Timing) Complete() {
	t.Completed = int((time.Now().UnixNano() - t.Began) / 1000)
}
...
func (t *Timing) Trace() *httptrace.ClientTrace {
	return &httptrace.ClientTrace{
		DNSStart: func(httptrace.DNSStartInfo) {
			t.DNSStart = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		DNSDone: func(httptrace.DNSDoneInfo) {
			t.DNSEnd = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		ConnectStart: func(string, string) {
			t.ConnectStart = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		ConnectDone: func(string, string, error) {
			t.ConnectEnd = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		TLSHandshakeStart: func() {
			t.TLSStart = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		TLSHandshakeDone: func(tls.ConnectionState, error) {
			t.TLSEnd = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		WroteHeaders: func() {
			t.WroteHeaders = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		WroteRequest: func(httptrace.WroteRequestInfo) {
			t.WroteRequest = int((time.Now().UnixNano() - t.Began) / 1000)
		},
		GotFirstResponseByte: func() {
			t.FirstResponseByte = int((time.Now().UnixNano() - t.Began) / 1000)
		},
	}
}
