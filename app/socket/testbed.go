...
func testbed(input json.RawMessage, connSvc *websocket.Service) error {
	parsed := &tkv{}
	err := util.FromJSON(input, parsed)
	if err != nil {
...
func process(x *tkv, connSvc *websocket.Service) error {
	switch x.T {
	case "theme":
...
func onLog(level string, js json.RawMessage, svc *websocket.Service) error {
	content, err := util.FromJSONString(js)
	if err != nil {
