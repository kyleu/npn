...
func (o *Options) Merge(data util.ValueMap, logger util.Logger) *Options {
	if o == nil {
		return nil
	}
	edh := make([]string, 0, len(o.ExcludeDefaultHeaders))
	for _, dh := range o.ExcludeDefaultHeaders {
		edh = append(edh, util.MergeLog("options.excludeDefaultHeaders", dh, data, logger))
	}
	rcj := make([]string, 0, len(o.ReadCookieJars))
	for _, cj := range o.ReadCookieJars {
		rcj = append(rcj, util.MergeLog("options.readCookieJars", cj, data, logger))
	}
	...
	return &Options{
		...
		ReadCookieJars:        rcj,
		WriteCookieJar:        util.MergeLog("options.writeCookieJar", o.WriteCookieJar, data, logger),
		SSLCert:               util.MergeLog("options.sslCert", o.SSLCert, data, logger),
		UserAgentOverride:     util.MergeLog("options.userAgentOverride", o.UserAgentOverride, data, logger),
	}
}
