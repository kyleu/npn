...
type Prototype struct {
	...
	Query    queryparam.QueryParams `json:"query,omitempty"`
...
func (p *Prototype) Merge(data util.ValueMap, logger util.Logger) *Prototype {
	meth := p.Method
	if util.MergeNeeded(meth.Key) {
		meth = Method{Key: util.MergeLog("proto.method", meth.Key, data, logger)}
	}
	prot := p.Protocol
	if util.MergeNeeded(prot.Key) {
		prot = Protocol{Key: util.MergeLog("proto.protocol", prot.Key, data, logger)}
	}

	return &Prototype{
		Method:   meth,
		Protocol: prot,
		Domain:   util.MergeLog("proto.domain", p.Domain, data, logger),
		Port:     p.Port,
		Path:     util.MergeLog("proto.path", p.Path, data, logger),
		Query:    p.Query.Merge(data, logger),
		Fragment: util.MergeLog("proto.fragment", p.Fragment, data, logger),
