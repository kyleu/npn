...
func (f *Form) Merge(data util.ValueMap, logger util.Logger) Config {
	d := make(FormData, 0, len(f.Data))
	for _, dt := range f.Data {
		d = append(d, &FormEntry{
			K: util.MergeLog("body.form.data.k", dt.K, data, logger),
			V: util.MergeLog("body.form.data.v", dt.V, data, logger),
		})
	}

	s := f.str
	if len(s) > 0 {
		s = util.MergeLog("body.form.str", s, data, logger)
	}
