...
##
## Outputs:
##   - build/dist/npn_<version>_ios_framework.zip
##   - build/dist/npn_<version>_ios_app.zip
...
GOARCH=arm64 time gomobile bind -o build/dist/mobile_ios_arm64/npnServer.xcframework -target=ios github.com/kyleu/npn/app/cmd
echo "gomobile for iOS completed successfully, building distribution..."
cd "build/dist/mobile_ios_arm64/npnServer.xcframework"
zip --symlinks -r "../../npn_${TGT}_ios_framework.zip" .
...
xcodebuild -project "NPN.xcodeproj" -allowProvisioningUpdates
zip -r "$dir/../../build/dist/npn_${TGT}_ios_app.zip" .

cd "$dir/../.."
echo "Output written to ./build/dist/npn_${TGT}_ios_framework.zip and ./build/dist/npn_${TGT}_ios_app.zip"
